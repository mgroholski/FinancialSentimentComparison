...
class RandomForest:
    def __init__(self, data_folder="../data_processor/price_news_integrate", save_model=True):
        """
        ...
        self.model = None
        self.df = None
        self.price_features = ["Open", "High", "Low", "Close", "Adj_Close", "Volume", "News_flag"]
    ...
    def prepare_features(self):
        ...
        embeddings = np.vstack(self.df["Lexrank_summary"].apply(self.minilm.encode).values)
        embed_df = pd.DataFrame(embeddings, columns=[f"embed_{i}" for i in range(embeddings.shape[1])])

        # Scale numeric features
        scaler = StandardScaler()
        scaled_numeric = scaler.fit_transform(self.df[self.price_features + ["Scaled_sentiment"]])
        numeric_df = pd.DataFrame(scaled_numeric, columns=self.price_features + ["Scaled_sentiment"])

        X = pd.concat([numeric_df.reset_index(drop=True), embed_df.reset_index(drop=True)], axis=1)
