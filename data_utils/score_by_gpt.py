...
def get_sentiment(symbol, *texts):
    ...
    # print(num_text)
    text_content = " ".join(
        [f"### News to Stock Symbol -- {symbol}: {text}" for text in texts]
    )
    # print(text_content)
    ...
    try:
        ...
        print("request error: response error", e)
        sentiment_value = np.nan
        sentiments.append(sentiment_value)
        return sentiments
...
def from_csv_get_sentiment(df, symbol, saving_path, batch_size=4):
    # 排序， 使得没处理的数据在底部出现
    df.sort_values(
        by="Sentiment_gpt", ascending=False, na_position="last", inplace=True
    )
    ...
    for i in range(0, len(df), batch_size):
        ...
        # 跳过已处理的行（None)
        if (
            df.loc[i : min(i + batch_size - 1, len(df) - 1), "Sentiment_gpt"]
            .notna()
            .all()
        ):
            # print("Processed")
        ...
        print("Now row: ", i)
        texts = [
            df.loc[j, "Lsa_summary"] if j < len(df) else 0
...
def reproduce(folder_path, name, saving_path, batch_size):
    ...
    if df["Sentiment_gpt"].isnull().any():
        ...
        while batch_size_process > 0:
            ...
            df = from_csv_get_sentiment(
                df, symbol, os.path.join(saving_path, name + ".csv"), batch_size_process
            )
...
if __name__ == "__main__":
    ...
    # # # Method 1
    news_files = [
        file for file in os.listdir(news_folder_path) if file.endswith(".csv")
    ]
